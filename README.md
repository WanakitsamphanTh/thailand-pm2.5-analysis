...
## Conclusion
I have performed data analysis on 4 pollutant concentrations: PM2.5, PM10, O₃, and NO₂, in Bangkok between 2016 and 2026 with primary focus on PM2.5. The analysis reveals that all pollutant variables are positively correlated, suggesting that they are influenced by similar environmental conditions. The long-term trend of PM2.5 concentrations shows a decline until approximately the end of 2021, followed by an upward trend afterward. Collecting additional data of longer time period would be beneficial for investigating the broader trend. Seasonal behavior is also evident in the series, as indicated by the autocorrelation function over 365 lags and by time series decomposition. \
I also experimented forecasting using ARIMA and RNN. Each produced different results. While the ARIMA model captures the general direction and short-term fluctuations of the actual PM2.5 data; its forecasts appear overly smooth, with narrower fluctuations around the mean. In contrast, the predictions from the RNN model have wider variability. The predicted peaks and valleys tend to align with the real pattern with a slight temporal delay, indicating responsive ability to dynamic behaviors of PM2.5; however, the predicted changes differ from the observed data, which suggest that the model misalign the intensity of fluctuations. 
